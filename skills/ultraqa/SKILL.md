...
|------------|-----------|---------------|
| `/startup-harness:ultraqa --tests` | tests | All test suites pass |
| `/startup-harness:ultraqa --build` | build | Build succeeds with exit 0 |
| `/startup-harness:ultraqa --lint` | lint | No lint errors |
| `/startup-harness:ultraqa --typecheck` | typecheck | No TypeScript errors |
| `/startup-harness:ultraqa --custom "pattern"` | custom | Custom success pattern in output |
...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Task(subagent_type="startup-harness:qa-tester", model="sonnet", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ```
   Task(subagent_type="startup-harness:architect", model="opus", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ```
   Task(subagent_type="startup-harness:executor", model="sonnet", prompt="FIX:
...
User can cancel with `/startup-harness:cancel` which clears the state file.
