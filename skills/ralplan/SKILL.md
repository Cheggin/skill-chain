...
Ralplan is a shorthand alias for `/startup-harness:omc-plan --consensus`. It triggers iterative planning with Planner, Architect, and Critic agents until consensus is reached, with **RALPLAN-DR structured deliberation** (short mode by default, deliberate mode for high-risk work).

## Usage

```
/startup-harness:ralplan "task description"
```
...
```
/startup-harness:ralplan --interactive "task description"
```
...
```
/startup-harness:omc-plan --consensus <arguments>
```
...
8. *(--interactive only)* On approval: invoke `Skill("startup-harness:team")` for parallel team execution (recommended) or `Skill("startup-harness:ralph")` for sequential execution -- never implement directly
