...
```
/startup-harness:sciomc <goal>                    # Standard research with user checkpoints
/startup-harness:sciomc AUTO: <goal>              # Fully autonomous until complete
/startup-harness:sciomc status                    # Check current research session status
/startup-harness:sciomc resume                    # Resume interrupted research session
/startup-harness:sciomc list                      # List all research sessions
/startup-harness:sciomc report <session-id>       # Generate report for session
```

### Quick Examples

```
/startup-harness:sciomc What are the performance characteristics of different sorting algorithms?
/startup-harness:sciomc AUTO: Analyze authentication patterns in this codebase
/startup-harness:sciomc How does the error handling work across the API layer?
```
...
Task(subagent_type="startup-harness:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="startup-harness:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="startup-harness:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="startup-harness:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/startup-harness:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/startup-harness:sciomc AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="startup-harness:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="startup-harness:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="startup-harness:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="startup-harness:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="startup-harness:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="startup-harness:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="startup-harness:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/startup-harness:sciomc status` | Show current session progress |
| `/startup-harness:sciomc resume` | Resume most recent interrupted session |
| `/startup-harness:sciomc resume <session-id>` | Resume specific session |
| `/startup-harness:sciomc list` | List all sessions with status |
| `/startup-harness:sciomc report <session-id>` | Generate/regenerate report |
| `/startup-harness:sciomc cancel` | Cancel current session (preserves state) |
