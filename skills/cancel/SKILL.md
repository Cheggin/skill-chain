...
```
ToolSearch(query="select:mcp__plugin_startup-harness_t__state_clear,mcp__plugin_startup-harness_t__state_read,mcp__plugin_startup-harness_t__state_write,mcp__plugin_startup-harness_t__state_list_active,mcp__plugin_startup-harness_t__state_get_status")
```
...
`/startup-harness:cancel` follows the session-aware state contract:
...
```
/startup-harness:cancel --force
```

```
/startup-harness:cancel --all
```
...
3. If a `session_id` was supplied to `/startup-harness:cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omc/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
|------|-----------------|----------------|
| Autopilot | Yes (phase, files, spec, plan, verdicts) | `/startup-harness:autopilot` |
| Ralph | No | N/A |
| Ultrawork | No | N/A |
| UltraQA | No | N/A |
| Swarm | No | N/A |
