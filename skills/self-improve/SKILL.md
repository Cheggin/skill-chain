...
| Planning | Hypothesis → structured plan | startup-harness:planner | opus |
| Architecture Review | 6-point plan review | startup-harness:architect | opus |
| Critic Review | Harness rule enforcement | startup-harness:critic | opus |
| Execution | Implement plan + run benchmark | startup-harness:executor | opus |
| Git Operations | Atomic merge/tag/PR | startup-harness:git-master | sonnet |
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `startup-harness:git-master`:
  ```
...
Spawn N `startup-harness:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `startup-harness:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `startup-harness:critic` with the plan + harness rules:
...
For each approved plan, spawn `startup-harness:executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `startup-harness:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
4. Print summary report:
   ...
   ```
5. Run `/startup-harness:cancel` for clean state cleanup
